...
(** This chapter is a quick guided tour of some basic features
  ...
  Verifiable C Reference Manual, [vst/doc/VC.pdf] in your VST installation;
...
(** To verify a C program, such as [sumarray.c], use the CompCert
  ...
  [clightgen] is one of the CompCert tools, built by mentioning
  the -clightgen option when you run ./configure when building CompCert.

  [clightgen] will produce a file [sumarray.v] that contains the Coq
...
(** The data types used in a functional model can be any kind of mathematics
  ...
  Notice that this functional spec does not depend on [sumarray.v] or even 
  on anything in the Verifiable C libraries. This is typical, and desirable:
...
(** The [ident*funspec] begins, [DECLARE _sumarray], that is where
 [_sumarray: ident] is an abbreviation for the positive number that
 CompCert uses to stand for this C identifier.  You can see the
 Definition of [_sumarray] near the beginning of [sumarray.v].

  A function is specified by its _precondition_ and its _postcondition_.
  The [WITH] clause quantifies over Coq values that may appear in both the 
  precondition and the postcondition. The precondition is parameterized by
  the C-language function parameters (in this case [_a, _n]) and the 
  postcondition is parameterized by a identifier [ret_temp], which is short
  for, ``the temporary variable holding the return value.''

  Function preconditions, postconditions, and loop invariants are _assertions_
  about the state of variables and memory at a particular program point.
  In an assertion [PROP(P) LOCAL(Q) SEP(R)], the propositions in the 
  sequence [P] are all of Coq type [Prop].  They describe things that 
  are true independent of program state.  In the function precondition
  above, the statement [0 <= size <= Int.max_signed] is true _just within
  the scope of the quantification of the variable_ [size]; that variable
  is bound by [WITH], and spans the [PRE] and[POST] assertions.

  The [LOCAL] propositions [Q] are _variable bindings_ of type [localdef].
  Here, the function-parameters [a] and [n] are treated as nonaddressable 
  local variables, or ``temp'' variables.  The localdef [temp _a  a]
  says that (in this program state) the contents of C local variable [_a]
  is the Coq value [a].  In general, a C scalar variable holds something 
  of type [val]; this type is defined by CompCert as, *)
Print val.
(*
 Inductive val: Type :=  
 | Vundef: val  
...
(** The [SEP] conjuncts [R] are _spatial assertions_ in separation logic.
  In this case, there's just one, a [data_at] assertion saying that at
  address [a] in memory, there is a data structure of type 
  ...
  [PROP] statements in this postcondition--no forever-true facts hold now,
  that weren't already true on entry to the function.

  The [LOCAL] clause _must not mention_ the function parameters or local
  variables, because they are destroyed on function exit (and because your
  local variable names will not be meaningful to the function's caller); 
  it will only mention the return-temporary [ret_temp].
  ...
  So, overall, the specification for [sumarray] is this:
  ``At any call to sumarray, there exist values [a, sh, contents, size]
  such that [sh] gives at least read-permission; [size] is representable
  as a nonnegative 32-bit signed integer; function-parameter [_a] contains
  value [a] and [_n] contains the 32-bit representation of [size];
  and there's an array in memory at address [a] with permission [sh]
...
Definition main_spec :=
 DECLARE _main
  ...
  POST [ tint ]  
     ...
     SEP(TT).
(** The postcondition says, we have indeed added up the global array
   [four]. *)
...
(** In Verifiable C's signed integer arithmetic, you must prove (if the 
 ...
 In general, for a claim about [Int.repr(x)] to be [useful], one also needs
 that [0 <= x <= Int.max_unsigned] or [Int.min_signed <= x <= Int.max_signed].
 The caller of this function will probably need to prove 
...
(** To prove the correctness of a whole program, 
  1. Collect the function-API specs together into 
       [Gprog: list (ident*funspec)]
...
(** What's in [Gprog] are the funspecs that we built using [DECLARE], 
  _in addition to_ some other funspecs that are part of the standard library.
  The tactic [with_library] augments the user-provided list with
  these additional specifications.
  (See also: the [with_library] chapter of VC.pdf)  *)
...
(** Now comes the proof that [f_sumarray], the body of the [sumarray()]
   function, satisfies [sumarray_spec], in the global context [(Vprog,Gprog)].
*)
...
(** Here, [f_sumarray] is the actual function body (AST of the C code) 
  ...
  You can read [body_sumarray] as saying: In the context of [Vprog] and
...
(** The predicate [semax_body] states the Hoare triple of the function body,
 ...
 To prove this, we begin with the tactic [start_function],  which takes 
 care of some simple bookkeeping and expresses the Hoare triple to be proved. *)
...
(** Some of the assumptions you now see above the line are,
  - [a, sh, contents, size], taken directly from the WITH clause 
       of [sumarray_spec];
  - [Delta_spec], the context in which Floyd's proof tactics will look up
  ...
  - [SH,H,H0], taken exactly from the [PROP] clauses of [sumarray_spec]'s
       precondition;
  - [POSTCONDITION],  taken exactly from the postcondition 
          of [sumarray_spec].
*)
...
(** We do Hoare logic proof by forward symbolic execution.
    You can see the current proof goal, at the beginning of
    the function body.  In a forward Hoare logic proof of 
...
style of proof rule to derive [Q].  Let us now apply
the [forward] tactic: *)

forward.  (* i = 0; *)

(** Look at the precondition of the current proof goal, that is,
  the second argument of [semax]; it has the form 
  [PROP(...) LOCAL(...) SEP(...)].  That precondition is also
  the _postcondition_ of the [i=0;] statement!  It's also much
  like the _precondition_ [i=0;] except for one change:  we now
  know that [i] is equal to [0], which is expressed in the [LOCAL]
  part as [temp _i (Vint (Int.repr 0))]. *)
...
(** The hint suggests using [abbreviate_semax], which will undo
...
(** SEE ALSO: VC.pdf Chapter 14 (_PROP LOCAL SEP_)
     and Chapter 15 (_Entailments_) *)

(** This proof goal is an _entailment_,  [ENTAIL Delta, P |-- Q],
   meaning "in context [Delta], any state that satisfies [P] will
   also satisfy [Q]."  

  In this case, the right-hand-side of this entailment is existentially
  quantified; it says: there exists a value [i] such that (among
  other things) [temp _i (Vint (Int.repr i))], that is, the C variable
  [_i] contains the value [i].  But the left-hand-side of the
  entailment says [temp _i (Vint (Int.repr 0))], that is, the C variable
  [_i] contains 0.
...
(** The required information to prove [Zlength contents = size]
    comes from the _precondition_ of the current [semax] goal.  
...
(* ================================================================= *)
(** ** Tying all the funtions together *)
...
(** The C program may do input/output, affecting the state of the
  ...
  any input/output, so we can use a trivial [Espec].  We provide
  this to the [semax_prog] proofs (below, in the [prog_correct] lemma)
...
(** The [semax_func] proof goal here has a list of function bodies,
  ...
  Each call to the [semax_func_cons] tactic uses a (previously proved)
  [semax_body] theorem to match up a function body to its funspec.
*)
...
(** Recommended: read VC.pdf Chapters 22-45, (up to _Pointer comparisons_) *)

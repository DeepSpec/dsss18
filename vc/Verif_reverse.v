...
(** As usual, we import the Verifiable C system [VST.floyd.proofauto],
  then the program to be verified, in this case [reverse].  Then we give
  the standard boilerplate definitions of [CompSpecs] and [Vprog]. *)
...
(** We will define a separation-logic predicate, [listrep sigma p],
 to describe the concept that the address [p] in 
memory is a linked list that represents
 the mathematical sequence [sigma].  Here, [sigma] is a list of [val],
...
(** This says, if [sigma] has head [h] and tail [hs], then
  ...
  But if [sigma] is [nil], then [p] is the null pointer,
  and the memory footprint is empty ([emp]).  The fact [p=nullval] is a pure proposition (Coq [Prop]); we inject this
  into the assertion language (Coq [mpred]) using the [!!] operator.
  ...
  [*] to combine it with [emp];  [!!P] has no specification to separate
  from.  Instead, we use the ordinary conjunction [&&].
*)

(** Now, we want to prevent the [simpl] tactic from
 automatically unfolding [listrep].  This is a
 design choice that you might make differently,
 in which case, leave out the [Arguments] command. *)
...
(** Whenever you define a new spatial operator--a definition
  of type [mpred] such as [listrep]--it's useful to populate
  two hint  databases.
...
Proof.
intros.
 (** This means [p] is a non-null pointer, not NULL or Vundef or a floating-point number: *)
Print isptr.
...
(** Let's look more closely at how [entailer!] solves this goal.  
     First, it finds all the pure propositions [Prop] that it can deduce from  the
     [mpred] conjuncts on the left-hand side, and puts them above the line. *)
saturate_local.
(** The [saturate_local] tactic uses a Hint database (also called [saturate_local])
...
- (** In the inductive case, we can again unfold the definition
 ...
 (** Warning!  Sometimes [entailer!] is too aggressive.  If we use it here,
  it will throw away the left-hand side because it doesn't understand how to look inside an 
  EXistential quantitier.
  The exclamation point [!] is a warning that [entailer!] can turn a provable
  goal into an unprovable goal.  The less aggressive version is without
  the exclamation point:  [entailer].  *)
 entailer.
 (** Notice that the existentially bound variable [y] has been moved above the line. *)

 (** In general, should you use [entailer!] or [entailer]  in ordinary proofs?  Usually [entailer!] is best: it's 
  faster, and it does more work for you.  Only if you 
  find that [entailer!] has gone into a dead end, should
  you use [entailer] instead. *)

 (** Now it is safe to use [entailer!], because it understands both [data_at] and [listrep]. *)
 entailer!.
...
(** 
  - The WITH clause says, there is a value [sigma: list val] and a value [p: val], 
              visible in both the precondition and the postcondition.
  - The PREcondition says, 
     - There is a single function-parameter named [_p], whose C type is "pointer to struct list"
     - LOCAL:  The C local variable [_p] contains the Coq value [p];
  ...
  - The POSTcondition says, 
     ...
     - LOCAL: the "return temporary", a pseudovariable holdingthe 
           function's return value, holds the value [q]
...
start_function.
(** The [semax] judgment has four parts:
  - [Delta], a "type context" ([tycontext]) that contains information
    about the (C language) types of C variables, specifications ([funspec]s)
    of global functions, and so on.
  - Precondition, in this example [(PROP ( )  LOCAL (temp _p p)  SEP (listrep sigma p))]
  - Command, in this example [ (_w = (tptr tvoid) (0); MORE_COMMANDS)]
  - Postcondition, in this example [POSTCONDITION]

 Notice that the precondition matches exactly the PRE clause of the
 [reverse_spec].  The postcondition matches too, but it's hidden inside
...
(** The command is what you would expect by reading the original C program.
  ...
  abbreviations [POSTCONDITION, MORE_COMMANDS] because, in a proof
...
(** The first statement (command) in the function-body is the assignment
     ...
     "cast 0 to void-pointer", [(void * )0], here ugly-printed as [(tptr tvoid)(0)].
    To apply the separation-logic assignment rule to this command, simply
     use the tactic [forward] : *)
forward.  (* w = NULL; *)
(** The new [semax] judgment is for the rest of the function body _after_
     the command [w=NULL].  The precondition of this [semax] is actually
     the postcondition of the [w=NULL] statement.  It's much like the
     precondition of [w=NULL], but contains the additional LOCAL fact,
     [temp _w (Vint (Int.repr 0))], that is, the local variable [_w] contains [nullval]. *)
...
(** As usual in any Hoare logic (including Separation Logic), the postcondition
    of a while-loop is {Inv /\ not Test}, where Inv is the loop invariant
    and Test is the loop test.  Here, all the EXistentials and PROPs of the
    loop invariant have been moved above the line as [s1,s2,w,v,HRE,H].

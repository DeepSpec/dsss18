...
let node_spec arg nodes_ref doc =
  let parse opt =
    ...
    if string_match (regexp "\\([^,]+\\),\\(.+\\):\\([0-9]+\\)") opt 0 then
      match LockServSerialization.deserialize_name (matched_group 1 opt) with
...
let parse inp =
  let opts =
    ...
    ; ("-me", Arg.String (fun opt -> 
      match LockServSerialization.deserialize_name opt with 
      | Some nm -> me := nm
...
let validate () =
  ...
  if not (mem_assoc !me !cluster) then
    raise (Arg.Bad (sprintf "%s is not a member of this cluster" (LockServSerialization.serialize_name !me)));

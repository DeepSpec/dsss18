...
Definition itree_server := M SocketInterface.SocketAPI.SocketE unit.

(* Type of trees annotated with traces. *)
Definition itree_traces :=
  M (discardE +' nondetE +' arbitraryE +' note_traceE) unit.
...
CoFixpoint enum_traces'
  ...
  | Vis X e k =>
    ...
    | (| e ) =>
      ...
      | Listen _ => fun id => Tau (enum_traces' st cur_trace (k (id tt)))
      | Accept _ => fun id =>
      ...
      | Shutdown _ => fun _ => discard "not implemented"
      end (fun x => x)
    | (| Fail reason |) => discard reason
    | ( _Or ||) =>
      match _Or in Common.nondetE X return (X -> _) -> _ with
      | Common.Or => fun id =>
        or (enum_traces' st cur_trace (k (id true)))
           (enum_traces' st cur_trace (k (id false)))
      end id
    end
  end.
...
Fixpoint qc_check (max_depth : nat)
  ...
  | S max_depth =>
    match t with
    | Tau t => qc_check max_depth check_trace t
    | Ret tt => ok
    | Vis _ e k =>
      match e with
      | (| _Note ) =>
        ...
        | NoteTrace tr => fun id cont fuel =>
          ...
          | Found => qc_check max_depth check_trace (k (id tt)) cont fuel
      ...
      | (| _Arb |) =>
        ...
        | Arb _ _ _ _ _ => fun id cont fuel =>
          checker (x <- arbitrary;;
                   ret (qc_check max_depth check_trace (k (id x)) cont fuel))
        end id
      | (| _Or ||) =>
        match _Or in nondetE X return (X -> _) -> _ with
        | Or _ n => fun id cont fuel =>
          checker (xs <- shuffle every_fin;;
                   ret (traverse_qc xs (fun x => qc_check max_depth check_trace (k (id x))) cont fuel))
        end id
      | ( _Discard |||) => ok
      end
    end
...
Definition check_trace_incl
  ...
  run_checker' backtrack_fuel
               (qc_check max_depth check_trace (enum_traces impl)).

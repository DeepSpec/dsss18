...
From DeepWeb.Proofs.Vst
     Require Import VstInit MonadExports VstLib.
...
Definition socket_spec (T : Type) :=
  ...
  POST [ tint ]
    ...
    EX r : Z,
    PROP ( r = YES \/ r = NO;
           r = YES -> (exists fd, r = descriptor fd /\
...
Definition bind_spec (T : Type) :=
  ...
  PRE [ 1%positive OF tint,
    ...
    SEP ( SOCKAPI st ;
          data_at Tsh (Tstruct _sockaddr noattr) (rep_sockaddr addr) addr_ptr
        )
  POST [ tint ]
    EX st' : SocketMap,
    EX r : Z, 
...
Definition listen_spec (T : Type) :=
  ...
  POST [ tint ]
    ...
    LOCAL ( temp ret_temp (Vint (Int.repr r)) )
    SEP ( SOCKAPI st';
          TRACE ( match result with Some tt => k | None => t end )
        ).
...
Definition recv_msg_spec (T : Type) :=
  ...
  PRE [ 1%positive OF tint, 2%positive OF (tptr tuchar),
    ...
    PROP ( consistent_world st;
           lookup_socket st fd = ConnectedSocket client_conn;
           writable_share sh ;
           trace_incl (r <- recv client_conn ;; k r) t
       )
    LOCAL (temp 1%positive (Vint (Int.repr (descriptor fd)));
           temp 2%positive buf_ptr;
...
Ltac forward_socket :=
  match goal with
  | [ H: trace_incl ((open_socket _ _ _) ;; ?k) ?t |- _ ] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
      forward_call (t, k, st)
    end
...
(*  match goal with
  | [|- context[TRACE ( (open_socket _ _ _) ;; ?k) :: _]] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
      forward_call (k, st)
    end
...
(*
Ltac forward_bind_and_listen :=
  match goal with
  | [|- context[TRACE (bind (bind_and_listen ?fd) ?k) :: _]] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
      forward_call (k, st, fd)
    end
  end.
 *)

Ltac forward_listen fd :=
  match goal with
  | [H: trace_incl (bind (listen ?addr) ?k) ?t |- _] =>
    match goal with
    | [|- context[SOCKAPI ?st]] =>
      forward_call (t, k, addr, st, fd)
    end
  end.

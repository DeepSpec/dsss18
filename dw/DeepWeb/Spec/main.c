...
int recv_msg(int fd, uint8_t* buffer_ptr, uint32_t len, int flags) {
  return recv(fd, buffer_ptr, len, flags);
}

int send_msg(int fd, uint8_t* buffer_ptr, uint32_t len, int flags) {
  return send(fd, buffer_ptr, len, flags);
}
...
conn_read(connection* conn, store* last_msg_store) {
  ...
  r = recv_msg(conn_fd, ptr, BUFFER_SIZE - request_len, 0);
  uint32_t new_len;
...
static int conn_write(connection* conn) {
  ...
  r = send_msg(conn_fd, response, num_bytes_to_send, 0);

...
(** We will need one identifier-specific operation, [fresh]: given any
    finite set of identifiers, we can produce one that is distinct from
    all other identifiers in the set. 
   
    To compute a fresh [id] given a list of [id]s
...
(** To generate identifiers for TImp, we will not just 
    generate arbitrary natural numbers. More often than not, 
    we will need to generate a set of identifiers, or pick 
    an identifier from such a set. If we represent a set as 
    a list, we can do the former with a recursive function
    that generates [n] fresh [nat]s starting from the empty 
    list. For the latter, we have QuickChick's [elements] 
    combinator. *)
...
(** Write a [Gen] instance for [id] using the [elements]
...
(** We will see in a different chapter ([QuickChickTool]) how 
    we can use QuickChick to introduce such _mutations_ and have them
